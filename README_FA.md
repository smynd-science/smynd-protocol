...
## دفترکل پاداش استفاده مجدد از داده پژوهشی

این ریپو اکنون یک لایه آزمایشی برای **Research Contribution Royalty Ledger** دارد. ایده این است که اگر در آینده یک دیتاست تأییدشده دوباره استفاده شد، رکوردهای مشارکت واجد شرایط بتوانند سهم پاداش مجدد دریافت کنند.

در نسخه v0.1 همه چیز با حفظ حریم خصوصی طراحی شده است:

- داده خام کاربر در سیستم اصلی Smynd می‌ماند؛
- manifest دیتاست فقط commitmentها را نگه می‌دارد؛
- usage event استفاده مجاز از دیتاست را ثبت می‌کند؛
- reward ledger سهم پاداش را بین رکوردهای واجد شرایط تقسیم می‌کند؛
- فقط Merkle root می‌تواند به صورت اختیاری روی زنجیره ثبت شود.

مستند اصلی: [`docs/royalty-ledger.md`](docs/royalty-ledger.md)
